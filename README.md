...
CODESTAR is a coding bootcamp in Bangkok,
specialized in Java and JavaScript.
Our objective is to train people to
get a job in leading enterprises in Thailand
and some world-class companies such as 
Microsoft, Amazon, Google, Apple and Facebook.

In order to achieve this goal, we designed our lessons
to meet the expectations carefully. For example, 
our homework are unique and impossible to search for 
the solution from any book or web site, every homework 
must be done by understanding of lesson.

CODESTAR เป็น Coding Bootcamp ในกรุงเทพ
เป้าหมายของเราคือการฝึกคนให้พร้อมไปทำงานในบริษัทชั้นนำของไทย
รวมถึงบริษัทระดับโลกอย่าง Microsoft, Amazon, Google, Apple และ Facebook

เพื่อให้ทุกอย่างเป็นไปอย่างที่ตั้งใจไว้ 
บทเรียนและแบบฝึกหัดจึงถูกคิดค้นขึ้นมาอย่างดี
ไม่มีทางที่จะหาคำตอบจากหนังสือหรือเว็บไซต์ที่ไหนได้
คนที่มาเรียนต้องใช้ความเข้าใจอย่างแท้จริงในการทำแบบฝึกหัด
...
- ให้เขียนโค้ดเพื่อรับ Binary Tree เข้ามา แล้วหาค่าใน Leaf ที่สูงที่สุด 
ตัวอย่างในภาพได้ผลลัพธ์คือ 9

- ให้เขียนโค้ดเพื่อรับ Binary Tree เข้ามา แล้วเอาค่าใน Leaf มารวมกัน 
ตัวอย่างในภาพได้ผลลัพธ์คือ 23

- ให้เขียนโค้ดเพื่อรับ Binary Tree เข้ามา แล้วหาค่าที่มากที่สุดของแต่ละชั้น 
ตัวอย่างในภาพได้ผลลัพธ์คือ 3, 8, 5, 9, 1

- ให้เขียนโค้ดเพื่อรับ Binary Tree เข้ามา แล้วหาผลรวมของแต่ละชั้น 
ตัวอย่างในภาพได้ผลลัพธ์คือ 3, 15, 14, 19, 1
...
เช่นมีตัวเลข 3, 2, 4, 7, 4, 9 เป็นไปได้หรือไม่ที่จะทำให้รวมกันเป็น 19 คำตอบคือ 
ทำได้ เช่น 2+4+4+9
...
ไม่จำเป็นต้องหยิบมาใช้ทุกใบ หยิบมาแค่บางใบ 
แต่ถ้าหยิบใบไหนมาแล้วเลือกใช้ได้แค่ค่าเดียวของไพ่ใบนั้น
...
ให้เขียนโค้ดเพื่อหมุนให้ขาที่ติดกัน มีสีหรือตัวเลขเดียวกัน ให้พิมพ์จำนวนการหมุนที่น้อยที่สุดออกมาถ้าเป็นไปได้ 
หรือพิมพ์ Impossible ถ้าเป็นไปไม่ได้
...
Merging two consecutive stacks of X containers and Y containers 
takes 2 * X * Y minutes. For example you have two stacks with 3 and 4 containers, 
it will take 2*(3+4) = 14 minutes. Write code to merge all stacks of containers 
by merging two consecutive stacks to a single stack in the minimum amount of time.

For example, you have stacks [4,2,3] 

You can merge in two ways: |4-2|-3 and 4-|2-3|. The first way takes 2*(4+2) 
= 12 minutes to become [6,3] and 2*(6+3) = 18 minutes for the second merge. 
Total time is 30 minutes.

The second way is to take 2*(2+3) = 10 minutes to become [4,5] and 2*(4+5) 
= 18 minutes, the total time is 28 minutes which is faster than the first way.
